...
assembleTandemDuplicationDF <- function(CNV="SERVER-OUTPUT/Output-WTvarWidthRef/BROWSERFILES/method-HMM/binsize_1e+05_stepsize_1e+05_StrandSeq_CNV.bed.gz",
  ...
  write.table(CNVgene,paste0(datapath,"/CNV_with_gene.txt"),quote=F,row.names = F,col.names = T,sep="\t")
  write.table(haploidPerCell,paste0(datapath,"/haploidPerCell.txt"),quote=F,row.names = F,col.names = T,sep="\t")
  write.table(haploidTotal,paste0(datapath,"/haploidTotal.txt"),quote=F,row.names = F,col.names = T,sep="\t")
  write.table(diploidPerCell,paste0(datapath,"/diploidPerCell.txt"),quote=F,row.names = F,col.names = T,sep="\t")
  write.table(diploidTotal,paste0(datapath,"/diploidTotal.txt"),quote=F,row.names = F,col.names = T,sep="\t")
  write.table(tandemRepeatsTotal_filtHighPloidy,paste0(datapath,"/TD_total_filtHighPloidy.txt"),quote=F,row.names = F,col.names = T,sep="\t")
  write.table(tandemRepeatsPerCell,paste0(datapath,"/tandemRepeatsPerCell.txt"),quote=F,row.names = F,col.names = T,sep="\t")
